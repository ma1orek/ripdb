...
import { Search, Zap, TrendingUp, Loader2 } from 'lucide-react';
...
export function SearchBar({ onActorSelect, placeholder = "Search actors..." }: SearchBarProps) {
  ...
  const { actors, searchActors, loading: dataLoading } = useGoogleSheets();
  ...
  // Get top actors for suggestions
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto">
      ...
      {showSuggestions && !query && !dataLoading && topActors.length > 0 && (
        <div className="mt-6 space-y-4 fade-in-up">
          <div className="text-center">
            <h3 className="text-xl font-medium text-white mb-2">Top Death Champions</h3>
            <p className="text-muted-foreground">From your live Google Sheets database</p>
          </div>
          
          <div className="grid md:grid-cols-3 gap-4">
            {topActors.slice(0, 3).map((actor, index) => (
              <div
                key={actor.id}
          ...
          {topActors.length > 3 && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-4">
              {topActors.slice(3).map((actor, index) => (
                <div
                  key={actor.id}
                  onClick={() => handleSelect(actor.id)}
